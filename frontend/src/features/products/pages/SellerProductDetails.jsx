...
const SellerProductDetails = () => {
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#131313] flex items-center justify-center text-[#fff6df] font-manrope">Loading gallery...</div>;
  }

  if (!product) {
    return <div className="min-h-screen bg-[#131313] flex items-center justify-center text-[#fff6df] font-manrope">Product Not Found</div>;
  }
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 mt-32">
        ...
        <section className="flex flex-col md:flex-row gap-8 mb-16">
          <div className="w-full md:w-1/2">
            ...
            <div className="w-full aspect-[4/5] bg-[#1c1b1b] overflow-hidden rounded-xl">
              {product.images && product.images.length > 0 ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-[#d0c6ab]">No Image</div>
              )}
            </div>
            {/* Thumbnails */}
            {product.images && product.images.length > 1 && (
              <div className="flex gap-2 mt-4 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-[#1c1b1b]">
                {product.images.slice(1).map((img, i) => (
                  <img key={i} src={img.url} alt={`Thumb ${i}`} className="w-20 h-24 object-cover bg-[#1c1b1b] shrink-0 rounded-lg" />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="w-full md:w-1/2 flex flex-col justify-center">
            <h2 className="font-extrabold text-4xl md:text-5xl leading-tight mb-4 tracking-tighter text-[#fff6df] uppercase">{product.title}</h2>
            <p className="text-[#d0c6ab] font-light text-lg mb-6 leading-relaxed max-w-md">{product.description}</p>
        ...
        <section className="bg-[#1c1b1b] p-6 md:p-12 rounded-xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <h3 className="font-extrabold tracking-tighter text-[#fff6df] text-3xl uppercase">Variants & Inventory</h3>
            {!isAddingVariant && (
              <button
          ...
          {isAddingVariant && (
            <div className="bg-[#131313] p-6 md:p-8 mb-12 shadow-2xl rounded-xl border border-[#2a2a2a]">
              <div className="flex justify-between items-center mb-8 border-b border-[#2a2a2a] pb-4">
                <h4 className="font-extrabold text-[#fff6df] tracking-tight text-xl uppercase">Create Variant</h4>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  {/* Dynamic Attributes */}
                  <div>
                    <label className="block text-xs uppercase tracking-widest font-bold text-[#d0c6ab] mb-4">Attributes (e.g. Size, Color) *</label>
                      ...
                      {attributeInputs.map((attr, index) => (
                        <div key={index} className="flex gap-4 items-center">
                          ...
                          {attributeInputs.length > 1 && (
                            <button onClick={() => handleRemoveAttribute(index)} className="text-red-400 p-3 bg-red-400/10 rounded-lg hover:bg-red-400/20 transition-colors cursor-pointer shrink-0">
                              <TrashIcon />
                            </button>
                          )}
                        </div>
                  ...
                  <div className="flex gap-6">
                    <div className="w-1/2">
                      <label className="block text-xs uppercase tracking-widest font-bold text-[#d0c6ab] mb-2">Initial Stock</label>
                      <input
                        type="number"
                    ...
                    <div className="w-1/2">
                      <label className="block text-xs uppercase tracking-widest font-bold text-[#d0c6ab] mb-2">Price Amount (Optional)</label>
                      <input
                        type="number"
                ...
                <div>
                  <div className="flex justify-between items-end mb-4">
                    <label className="block text-xs uppercase tracking-widest font-bold text-[#d0c6ab]">Image Upload (Max 7)</label>
                    <span className="text-xs text-[#ffd700] bg-[#ffd700]/10 px-2 py-1 rounded">{newVariant.images.length}/7</span>
                  </div>

                  {newVariant.images.length > 0 && (
                    <div className="grid grid-cols-3 gap-3 mb-6">
                      {newVariant.images.map((img, index) => (
                        <div key={index} className="relative aspect-[4/5] bg-[#1c1b1b] rounded-lg overflow-hidden group">
                          <img src={img.previewUrl} alt="Preview" className="w-full h-full object-cover group-hover:opacity-75 transition-opacity" />
                          <button
          ...
          {localVariants.length === 0 ? (
            <div className="py-16 text-center text-[#d0c6ab] bg-[#131313] rounded-xl border border-[#2a2a2a] border-dashed">
              <p className="font-light tracking-wide">No variants have been created yet.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {localVariants.map((variant, idx) => (
                <div key={idx} className="bg-[#131313] flex flex-col pt-5 rounded-xl border border-[#2a2a2a] overflow-hidden hover:border-[#ffd700]/30 transition-colors">
                  <div className="px-6 flex gap-5 h-24 mb-6">
                    ...
                    <div className="w-16 h-20 bg-[#1c1b1b] shrink-0 rounded-lg overflow-hidden border border-[#2a2a2a]">
                      {variant.images && variant.images.length > 0 ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-xs text-[#d0c6ab]">N/A</div>
                      )}
                    </div>
                    {/* Attributes */}
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap gap-2 mb-3">
                        {Object.entries(variant.attributes || {}).map(([ key, val ]) => (
                          <span key={key} className="bg-[#1c1b1b] px-2.5 py-1 text-[10px] rounded uppercase tracking-widest text-[#fff6df] border border-[#2a2a2a]">
                            <span className="text-[#d0c6ab]/70 mr-1">{key}:</span>{val}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="mt-auto border-t border-[#2a2a2a] bg-[#1c1b1b]/50 flex items-center px-6 py-4 justify-between">
                    <label className="text-xs text-[#d0c6ab] uppercase tracking-widest font-bold">Current Stock</label>
