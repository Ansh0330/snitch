      ...
      <main>
        ...
        <section className="relative h-screen w-full flex items-center justify-center overflow-hidden bg-[#131313]">
          ...
          <div className="relative z-10 text-center px-6">
            <p className="text-[0.65rem] uppercase tracking-[0.6em] text-[#d0c6ab] mb-6 font-bold drop-shadow-2xl">
              Limited Release
            </p>
        ...
        <section className="max-w-[1920px] mx-auto px-6 md:px-10 py-32 bg-[#131313]">
          <div className="flex flex-col md:flex-row justify-between md:items-end mb-16 gap-6">
            <div className="space-y-4">
              <h2 className="text-4xl md:text-5xl font-extrabold tracking-tighter text-[#fff6df]">
                The Curated Edit
              </h2>
            ...
            {allProducts?.length > 0 && (
              <button
                onClick={() => navigate("/artistry")}
                className="text-[#d0c6ab] text-[0.65rem] uppercase tracking-[0.3em] font-bold border-b border-[#4d4732] pb-1 hover:text-[#ffd700] hover:border-[#ffd700] transition-all cursor-pointer self-start md:self-auto"
              >
                Explore All Pieces
              </button>
            )}
          ...
          {allProducts && allProducts.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0 border border-[#1c1b1b]">
              {allProducts.map((product) => (
                <article
                  ...
                  key={product._id}
                  className="group relative aspect-[3/4] bg-[#1c1b1b] border-r border-b border-[#4d4732]/20 overflow-hidden cursor-pointer"
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-8 transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500">
                    <p className="text-[0.55rem] uppercase tracking-[0.3em] font-bold text-[#d0c6ab] mb-2">
                      {new Date(product.createdAt).toLocaleDateString(
                        undefined,
                        { year: "numeric" },
                      )}
                    </p>
                    <h3 className="text-2xl font-light text-[#fff6df] tracking-tight mb-2 line-clamp-1">
                      {product.title}
                    </h3>
                    <p className="text-[#ffd700] font-bold text-lg tracking-widest">
            ...
            <div className="w-full py-32 flex flex-col items-center justify-center border border-[#1c1b1b] bg-[#1c1b1b]/20">
              ...
              <p className="text-[#d0c6ab] text-xs max-w-sm text-center uppercase tracking-[0.2em] font-bold">
                The showroom is currently being curated.
              </p>
            </div>
          )}
        </section>
        ...
        <section className="bg-[#1c1b1b] py-32 px-10 border-t border-[#4d4732]/20">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            <span
              className="material-symbols-outlined text-[#ffd700] text-5xl opacity-80"
              style={{ fontVariationSettings: "'FILL' 1" }}
            >
              verified_user
            </span>
            <h2 className="text-4xl md:text-5xl font-light tracking-tight text-[#fff6df]">
              Luxe Is Not Heard. It Is Felt.
            </h2>
            <p className="text-[#d0c6ab] max-w-2xl mx-auto leading-relaxed font-light text-sm md:text-base">
              Our philosophy centers on the surgical application of
              craftsmanship. Every piece in the Pre-Fall collection is a
              testament to the power of pure restraint. Welcome to the new
              standard of premium.
            </p>
            <div className="pt-8">
              <button className="group inline-flex items-center gap-4 text-[#ffd700] uppercase tracking-[0.4em] text-[0.65rem] font-bold cursor-pointer">
                Read the Manifesto
                <span className="material-symbols-outlined transition-transform group-hover:translate-x-2 text-lg">
                  arrow_right_alt
                </span>
              </button>
            </div>
      ...
      <footer className="w-full py-16 bg-[#131313] border-t border-[#1c1b1b]">
        <div className="flex flex-col md:flex-row justify-between items-center px-12 max-w-[1920px] mx-auto text-[0.6rem] uppercase tracking-[0.2em] font-bold">
          ...
          <div className="flex flex-wrap justify-center gap-8 md:gap-12">
            <a className="text-[#d0c6ab] opacity-40 hover:text-[#ffd700] hover:opacity-100 transition-opacity cursor-pointer">
              Privacy Policy
            </a>
            <a className="text-[#d0c6ab] opacity-40 hover:text-[#ffd700] hover:opacity-100 transition-opacity cursor-pointer">
              Terms of Service
